...
| `--episode-voice-feedback` | **Opt-in episode announcements** With `--record`, speak “Starting recording” after an episode is created, “Stopping recording” when its save is requested, and “Recording saved” only after the asynchronous writer completes. Every tenth finalized episode in the current task directory also speaks the total (for example, “10 episodes saved”). Speech is queued outside the control loop. Bundled neural WAV prompts play through PipeWire with `pw-play`, including natural-voice totals from 10 through 1,000. A missing or out-of-range total is skipped rather than spoken by `spd-say`; `spd-say` remains only as a lifecycle-prompt fallback. |
